...
const CREATE_CONTROLLER = "gen-controller";
...
function generateBusiness($arguments) {
    ...
    $ignoreEntityCheck = in_array("ie", $arguments);
    ...
    $str = str_replace("<entity-name>", $arguments[1], $str);
...
function generateController($arguments) {
    ...
    $controllerPrefix = RUtil::getTextBetweenTags(Conf::getAppConfigProperty(Conf::CONF_NAMING_CONTROLLER_IMPLEMENTATION), "prefix");
    $controllerSuffix = RUtil::getTextBetweenTags(Conf::getAppConfigProperty(Conf::CONF_NAMING_CONTROLLER_IMPLEMENTATION), "suffix");

    $entityPrefix = RUtil::getTextBetweenTags(Conf::getAppConfigProperty(Conf::CONF_NAMING_PERSISTENCE_ENTITY), "prefix");
    $entitySuffix = RUtil::getTextBetweenTags(Conf::getAppConfigProperty(Conf::CONF_NAMING_PERSISTENCE_ENTITY), "suffix");

    $controllerImplementationName = $controllerPrefix . $arguments[1] . $controllerSuffix;
    $entityName = $entityPrefix . $arguments[1] . $entitySuffix;
    ...
    $controlerImplementationFile = join(DIRECTORY_SEPARATOR, array(__APP_PATH, "controllers", $controllerImplementationName . ".php"));
    if (file_exists($controlerImplementationFile) == true) {
        echo "\ncontroller class file already exists ({$controlerImplementationFile})";
        exit(-1);
    }

    $templateControllerImplementationFile = join(DIRECTORY_SEPARATOR, array("framework", "code-generator", "templates", "controller", "controller.php"));
    if (!copy($templateControllerImplementationFile, $controlerImplementationFile)) {
        echo "Error cloning file from template {$templateControllerImplementationFile}";
    }

    $str = file_get_contents($controlerImplementationFile);
    $str = str_replace("<class-name>", $controllerImplementationName, $str);
    $str = str_replace("<entity-name>", $arguments[1], $str);
    file_put_contents($controlerImplementationFile, $str);
}
